...
LogicalExpression -> 
  ...
  {% ([identifier,,operator,,number]) => {
    const propertyName = identifier.value;
    ...
    const evaluate = (obj) => {
      ...
      if (objValue === null || objValue === undefined) {
        throw new Error(`[Evaluation Failed] Property '${propertyName}' is not defined.`);
      }
      ...
      switch(operator) {
        ...
        case '<=': return objValue <= value;
        case '==': return objValue == value;
        default: throw new Error(`[Evaluation Failed] Relation Operator '${operator}' is not supported.`);
      }
    };

    return { evaluate, evaluateExp: `${propertyName} ${operator} ${value}` };
  } %}
